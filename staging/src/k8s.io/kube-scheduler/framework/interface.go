...
// IsUnschedulable returns true if and only if "Status" is non-nil and its Code is "Unschedulable".
...
type Domain interface {
	...
	GetAllPossibleVictims(nodeInfoLister NodeInfoLister) []PreemptionUnit
...
type PreemptionUnit interface {
	...
	AffectedNodes() map[string]NodeInfo
...
// PreemptionPluginInterface is expected to be implemented by different preemption plugins as all those member
// methods might have different behavior compared with the default preemption.
type PreemptionPluginInterface interface {
	...
	PodEligibleToPreemptOthers(ctx context.Context, pod *v1.Pod, nominatedNodeStatus *Status) (bool, string)
	...
	SelectVictimsOnDomain(ctx context.Context, preemptor Preemptor, domain Domain, pdbs []*policy.PodDisruptionBudget) ([]*v1.Pod, int, *Status)
...
type PreemptionEvaluator interface {
	Preempt(ctx context.Context, preemptor Preemptor, m NodeToStatusReader, pi PreemptionPluginInterface) (*PostFilterResult, *Status)
