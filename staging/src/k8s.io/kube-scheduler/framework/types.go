...
type PlacementInfo struct {
	// AncestorPlacements is an ordered list of ancestors,
	// with the initial placement being the first element,
	// and the immediate parent of the current placement being the last.
	// For initial placements, it's expected to be empty.
	AncestorPlacements []*Placement

	// Placement is the placement generated by the last placement generator plugin.
	// If no placement generator plugins have run yet, it's a placement that matches all nodes in a cluster
	Placement

	// PlacementNodes are nodes in the cluster that match the placement.
	PlacementNodes []NodeInfo
}