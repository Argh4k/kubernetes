...
func TestStrategyCreate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			description: "workload aware preemption enabled - preserve priorityClassName",
			workload: func() *scheduling.Workload {
				w := workload.DeepCopy()
		...
		{
			description: "workload aware preemption disabled - drop priorityClassName",
			workload: func() *scheduling.Workload {
				w := workload.DeepCopy()
				w.Spec.PodGroupTemplates[0].PriorityClassName = "high-priority"
				return w
		...
		{
			description: "workload aware preemption enabled - invalid priorityClassName",
			workload: func() *scheduling.Workload {
				w := workload.DeepCopy()
...
func TestStrategyUpdate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			description: "priorityClassName update, workload aware preemption disabled",
			oldWorkload: func() *scheduling.Workload {
				w := workload.DeepCopy()
				w.Spec.PodGroupTemplates[0].PriorityClassName = "high-priority"
				return w
		...
		{
			description: "priorityClassName update, workload aware preemption enabled",
			oldWorkload: func() *scheduling.Workload {
				w := workload.DeepCopy()
				w.Spec.PodGroupTemplates[0].PriorityClassName = "high-priority"
				return w
