...
func (e *Executor) prepareCandidateAsync(c Candidate, preemptor ExecutorPreemptor, pluginName string) {
	...
	go func() {
		...
		nominatedPods := getLowerPriorityNominatedPods(e.fh, preemptor.Priority(), c.Nodes())
...
func (e *Executor) prepareCandidate(ctx context.Context, c Candidate, preemptor ExecutorPreemptor, pluginName string) *fwk.Status {
	...
	// nomination updates these pods and moves them to the active queue. It
	// lets scheduler find another place for them sooner than after waiting for preemption completion.
	nominatedPods := getLowerPriorityNominatedPods(fh, preemptor.Priority(), c.Nodes())
...
func getLowerPriorityNominatedPods(pn fwk.PodNominator, priority int32, nodeNames []string) []*v1.Pod {
	podInfos := []fwk.PodInfo{}
	for _, n := range nodeNames {
		podInfos = append(podInfos, pn.NominatedPodsForNode(n)...)
	}
