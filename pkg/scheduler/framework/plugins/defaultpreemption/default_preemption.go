...
type DefaultPreemption struct {
	...
	// IsEligiblePod returns whether a victim (single pod/podGroup) is allowed to be preempted by a preemptor pod.
...
func (pl *DefaultPreemption) SelectVictimsOnDomain(
	...
	// If the preemptor does not fit after removing all the eligible pods,
...
func (pl *DefaultPreemption) isPreemptionAllowedForDomain(domain preemption.Domain, victim preemption.Victim, preemptor preemption.Preemptor) bool {
	// We are not assuming PodGroup as victim scenarios for now.
