...
func (s *Snapshot) TraverseHierarchyFromPod(
	pod *v1.Pod,
	...
	pgs, exists := s.podGroupStates[currentKey]
	if !exists {
	...
	if s.compositePodGroupEnabled && pg.Spec.ParentCompositePodGroupName != nil {
		...
		for range schedulingv1alpha3.WorkloadMaxTreeDepth - 1 {
			cpgs, exists := s.compositePodGroupStates[currentKey]
			if !exists {
	...
	visited := sets.New[fwk.EntityKey]()
	return s.traversePodGroupTree(currentKey, isCPGReady, isPGReady, visited)
}

func (s *Snapshot) traversePodGroupTree(
	key fwk.EntityKey,
	isCPGReady func(cpg *schedulingv1alpha3.CompositePodGroup, cpgState fwk.CompositePodGroupState, childrenResults []bool) bool,
	isPGReady func(pg *schedulingv1beta1.PodGroup, pgState fwk.PodGroupState) bool,
	...
	case fwk.CompositePodGroupKeyType:
		...
		for childKey := range children {
			res, err := s.traversePodGroupTree(childKey, isCPGReady, isPGReady, visited)
			if err != nil {
				return false, err
			}
