...
func (sched *Scheduler) podGroupCycle(ctx context.Context, schedFwk framework.Framework, podGroupInfo *framework.QueuedPodGroupInfo) {
	...
	useWAP := utilfeature.DefaultFeatureGate.Enabled(features.WorkloadAwarePreemption)
...
// workloadAwarePreemption runs workload-aware preemption for the given pod group.
// It saves the current state of the cluster snapshot, runs a PodGroupPreemptor.Preempt
